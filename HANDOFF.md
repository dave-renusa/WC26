...
- **⚠️ Scoring gotcha — knockouts NEVER tie (migration 010):** the 0.5 half-point "draw" rule is **group-stage only**. ESPN reports a knockout score as regulation+extra-time only, so a penalty-shootout game arrives as a draw on the scoreline (e.g. `1-1`) even though `winner_team_id` is correctly set to the shootout winner. `v_pick_scores` must check `m.stage = 'group'` before applying the 0.5 draw branch, and otherwise score off `winner_team_id` (full round points to the PK winner's backers, 0 to everyone else). A knockout result that looks like a draw on the scoreline is a penalty shootout — resolve it via `winner_team_id`, never as a tie.
